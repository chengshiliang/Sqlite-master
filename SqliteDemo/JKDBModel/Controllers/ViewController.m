...
- (IBAction)insertData:(id)sender {
    for (int i = 1; i <= 5; i++) {
        ...
        user.sex = [@"男" mutableCopy];
        user.account = @"344r345334543";
        user.portraitPath = @"werqwerwqer";
        user.moblie = [NSData dataWithBytes:"111222333" length:10];
        user.date = [NSDate date];
        user.age = 10+i;
        user.height = 10.000;
        user.array = @[@0,@1,@2];
        Depart *depart = [[Depart alloc]init];
        depart.departNum = @"111";
        depart.departName = @"222";
        NSArray<Depart *>*departs = @[depart];
...
- (IBAction)deleteData:(id)sender {
    [[[User alloc] init] deleteObjectsByCriteria:@" WHERE pk < 10"];
}
...
- (IBAction)queryData1:(id)sender {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSLog(@"第一条:%@",[User findFirstByCriteria:@" WHERE pk = 0 "]);
    });
}
